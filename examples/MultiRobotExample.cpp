...
void compute(size_t num_robots, string file_name, bool is_partition, string strongth = "strong")
{
  ...
  */
  ...
  {
    PoseID src = PoseMap[mIn.p1]; // i的相对位姿变换
    PoseID dst = PoseMap[mIn.p2]; // j的相对位姿变换

    unsigned srcRobot = src.first; //机器人的编号
    unsigned srcIdx = src.second;  //局部位姿索引
    unsigned dstRobot = dst.first;
    unsigned dstIdx = dst.second;

    RelativeSEMeasurement m(srcRobot, dstRobot, srcIdx, dstIdx, mIn.R, mIn.t,
                            mIn.kappa, mIn.tau);  //不确定度

    if (srcRobot == dstRobot)  //判断位姿属于所属类型
    {
      // private measurement
      if (mIn.p1 + 1 == mIn.p2)
      {
  ...
  ###########################################
  */
  vector<PGOAgent *> agents; //agent指针
  for (unsigned robot = 0; robot < (unsigned)num_robots; ++robot)
  {
    PGOAgentParameters options(d, r, num_robots);
    options.acceleration = acceleration;
    options.verbose = verbose; //是否cout
...
//文件保存命名
  ...
  if (is_graph_partition)
    ...
    filename = "../../result/graph/NP" + file_name;
  filename += ".txt";
